...
export const PaymentInfoCard = ({ item }) => {
  ...
  const lastFourDigitsOfBankNumber = bankNumber?.slice(-4);
  const maskedPartOfBankNumber = "*".repeat(bankNumber?.length - 4);

  const lastFourDigitsOfCardNumber = cardNumber?.slice(-4);
  const maskedPartOfCardNumber = "*".repeat(cardNumber?.length - 4);
  ...
  return (
    <div className="bg-gray-100 relative w-full px-4 py-4 dark:bg-darkMode dark:border-2 dark:border-dark-primary">
      ...
      <h4 className="md:text-xl font-semibold text-lg flex justify-start items-center space-x-2">
        <span>

...
export const AddForm = ({ setShowForm, updatedProperties }) => {
  ...
  const onSubmit = (values, { resetForm }) => {
    dispatch(
      addApartment({
        ...
        published: published,
        renovated: isChecked ? true : false,
      })
    );
    resetForm();
    setShowForm(false);
  ...
  if (updatedProperties && updatedProperties.length > 0) {
    propertyOptions = updatedProperties.map((item) => {
      ...
      {({ setFieldValue, touched, errors, values }) => {
        ...
        return (
          <Form className="w-full space-y-5">
            ...
            <div className="md:bg-gray-50 md:p-4 md:border md:border-blue-500 dark:md:bg-darkMode dark:md:border-dark-primary">
              <h3 className="font-medium">Add Fees(One Time/Monthly)</h3>
              <hr className="border-gray-300 mt-1 mb-3" />
                ...
                <div>
                  <div className="flex flex-wrap justify-start items-center gap-4">
                    {previewUrls2.slice(0, 0).map((url, index) => (
                      <div
                        key={index}
                        className="h-32 w-32 overflow-hidden border relative"
                      >
                        <LazyLoadImage
                          className="h-32 w-32 rounded-lg object-cover"
                          src={`${config.url.ASSET_URL}${url.link}`}
                    ...
                    {isModalOpenGallery && (
                      <Modal
                        title="Uploaded Apartment Gallery"
